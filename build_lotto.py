...
result = result.replace('__FREQ__', J(freq_list))
result = result.replace('__REC__', J(recent_freq))
result = result.replace('__DEC__', J(decade_data))
result = result.replace('__LAST20__', J(draws[-20:]))
result = result.replace('__PAIRS__', J(top_pairs))
result = result.replace('__TRIPLES__', J(top_triples))
result = result.replace('__MARKOV__', J(markov_top))
result = result.replace('__CLUSTERS__', J(clusters))
result = result.replace('__BOOT__', J(boot_ci))
result = result.replace('__RF__', J(rf_probs_out))
result = result.replace('__RF6__', J(rf_top6))
result = result.replace('__TOTAL__', str(total))
result = result.replace('__SPLIT__', str(split))
result = result.replace('__RFACC__', str(avg_acc))
result = result.replace('__RF6STR__', str(rf_top6))
result = result.replace('__LAST_DATE__', draws[-1]['date'])
result = result.replace('__DRAWS_ROWS__', draws_rows)
